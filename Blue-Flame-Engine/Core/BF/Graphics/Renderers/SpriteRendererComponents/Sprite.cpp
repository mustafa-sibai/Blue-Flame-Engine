		...
		namespace Renderers
		{
			namespace SpriteRendererComponents
			{
				...
				Sprite::Sprite(const Texture2D* texture2D, RenderLayer& renderLayer) :
					Renderable(Type::Sprite, Vector2i(texture2D->GetTextureData()->width, texture2D->GetTextureData()->height), Vector2f(0.5f), 0, renderLayer, Color::Whites::White()),
				...
				Sprite::Sprite(const Texture2D* texture2D, const Vector2f& pivot, RenderLayer& renderLayer) :
					Renderable(Type::Sprite, Vector2i(texture2D->GetTextureData()->width, texture2D->GetTextureData()->height), pivot, 0, renderLayer, Color::Whites::White()),
					texture2D(texture2D), scissorRectangle(0, 0, texture2D->GetTextureData()->width, texture2D->GetTextureData()->height)
				{
				}

				Sprite::Sprite(const Texture2D* texture2D, const Vector2f& pivot, int zSortingOrder, RenderLayer& renderLayer) :
					Renderable(Type::Sprite, Vector2i(texture2D->GetTextureData()->width, texture2D->GetTextureData()->height), pivot, zSortingOrder, renderLayer, Color::Whites::White()),
					texture2D(texture2D), scissorRectangle(0, 0, texture2D->GetTextureData()->width, texture2D->GetTextureData()->height)
				{
				}

				Sprite::Sprite(const Texture2D* texture2D, const Vector2f& pivot, int zSortingOrder, RenderLayer& renderLayer, const Color& color) :
					Renderable(Type::Sprite, Vector2i(texture2D->GetTextureData()->width, texture2D->GetTextureData()->height), pivot, zSortingOrder, renderLayer, color),
					texture2D(texture2D), scissorRectangle(0, 0, texture2D->GetTextureData()->width, texture2D->GetTextureData()->height)
				{
				}
