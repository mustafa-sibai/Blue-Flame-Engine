	...
	namespace Graphics
	{
		namespace Renderers
		{
			...
			Sprite::Sprite(const Texture2D* texture2D) :
				Renderable(Vector2i(texture2D->GetTextureData()->width, texture2D->GetTextureData()->height), Vector2f(0), 0, Color::Whites::White(), RenderableType::Sprite),
			...
			Sprite::Sprite(const Texture2D* texture2D, const Vector2f& pivot) :
				Renderable(Vector2i(texture2D->GetTextureData()->width, texture2D->GetTextureData()->height), pivot, 0, Color::Whites::White(), RenderableType::Sprite),
				texture2D(texture2D), scissorRectangle(0, 0, texture2D->GetTextureData()->width, texture2D->GetTextureData()->height)
			{
			}

			Sprite::Sprite(const Texture2D* texture2D, const Vector2f& pivot, unsigned int zLayer) :
				Renderable(Vector2i(texture2D->GetTextureData()->width, texture2D->GetTextureData()->height), pivot, zLayer, Color::Whites::White(), RenderableType::Sprite),
				texture2D(texture2D), scissorRectangle(0, 0, texture2D->GetTextureData()->width, texture2D->GetTextureData()->height)
			{
			}

			Sprite::Sprite(const Texture2D* texture2D, const Vector2f& pivot, unsigned int zLayer, const Color& color) :
				Renderable(Vector2i(texture2D->GetTextureData()->width, texture2D->GetTextureData()->height), pivot, zLayer, color, RenderableType::Sprite),
				texture2D(texture2D), scissorRectangle(0, 0, texture2D->GetTextureData()->width, texture2D->GetTextureData()->height)
			{
			}
			...
			Sprite::Sprite(const Texture2D* texture2D, const Vector2f& pivot, unsigned int zLayer, const Rectangle& scissorRectangle, const Color& color) :
				Renderable(Vector2i(scissorRectangle.width, scissorRectangle.height), pivot, zLayer, color, RenderableType::Sprite),
			...
			Sprite::Sprite(const Texture2D* texture2D, const Vector2f& pivot, const Vector2i& size, unsigned int zLayer, const Color& color) :
				Renderable(size, pivot, zLayer, color, RenderableType::Sprite),
				texture2D(texture2D), scissorRectangle(0, 0, texture2D->GetTextureData()->width, texture2D->GetTextureData()->height)
			{
			}
